"""
WhatsApp webhook endpoint for Vercel serverless deployment
"""
...
async def whatsapp_webhook(
    ...
    try:
        ...
        if not client_config:
            ...
            return WebhookResponse(status="ignored", message="unknown_instance")
        ...
        if event_type == 'messages.upsert':
            processed_count = await handle_message_event(client_config, event_data)
            ...
            await handle_connection_event(client_config, event_data)
            return WebhookResponse(status="processed", message="connection_updated")
        else:
            logger.info(f"Unhandled webhook event: {event_type}")
            return WebhookResponse(status="ignored", message="unhandled_event")
...
async def handle_message_event(client_config: Dict[str, Any], event_data: Dict[str, Any]) -> int:
    ...
    try:
        ...
        for msg_data in messages:
            ...
            asyncio.create_task(process_incoming_message(client_config, user_message))
            processed_count += 1
...
async def handle_connection_event(client_config: Dict[str, Any], event_data: Dict[str, Any]):
